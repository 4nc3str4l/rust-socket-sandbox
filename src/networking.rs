...
pub async fn network_processor(ui_to_network: &mut Receiver<Message>, network_to_ui: &mut Sender<Message>) {
    ...
    let mut connection_map: HashMap<String, SplitSink<WebSocketStream<MaybeTlsStream<TcpStream>>, tungstenite::Message>> = HashMap::new();
...
pub async fn handle_new_client(
    ...
    id: String,
    ip: String,
) -> Result<SplitSink<WebSocketStream<MaybeTlsStream<TcpStream>>, tungstenite::Message>, anyhow::Error> {
        ...
        while let Some(result) = read.next().await {
            match result {
                Ok(message) => {
                    ...
                    let _ = net_to_ui.send(Message::Message { id: idt.to_owned(),  payload: message_string }).await;
                }
                Err(e) => {
